...
- Create a 3D unity project (2021.3 preferred), and call it Dookie Dodger.
- Copy the `DookieStarterFiles` asset pack into the assets folder.
	- If you drag the pack into unity directly it should extract automatically.
	- If not, you can double click on the file in unity to extract the assets.
- Go into the scenes folder and open the 'DookieDodger' scene.

## Set up the Scene:
#### Road:
- Create a new plane, and call it 'ground'.
- Move it to (0, 0, 0), and scale it to (5, 5, 5).
	- (Tip: you can click on the chain icon next to the scale field to change them all at the same time).
- Go into the textures folder and drag the 'Road' texture on to the plane.
	- A new material was created in the 'materials' folder by doing this.

#### Power-lines:
- Go into the models folder, and drag the powerpole model into the scene next to the road.
- Make sure it is in a good position (-5, 0, -15) is best.
- Create a material in the 'materials' folder and call it 'Powerpole'.
- Change the colour of the new material to be appropriate for a power pole (e.g. brown).
- Drag the new material onto the power pole.
- Move the power pole to be next to the road.
- Duplicate the pole using ctrl+d when it is selected.
- Move the new pole 30 units along the road.
	- Tip: you can 'add' to numbers in fields instead of just changing them:
	- In the second poles position, click on the 'z' field (this will select the number), then click again (this will put the cursor in the field) then type '+30' at the end of the number.
...
- Add a line renderer object by going to GameObject->Effects->LineRenderer.
- Make the line renderer a child of the first power pole.
- Move it so that it aligns with one of the spikes
	- (3,-0.7,15) is a good position if they can't do it easily.
- A line renderer draws a line between each of the points you give it.
- In the line renderer component, change the second point to line up with the second power pole.
	- If you did the last step right this should be easy - just set the point to (0, 0, 30).
- The line looks right now except for the colour - lets create a new material for it just like we did for the power poles (black is probably a good colour).
- Now that the line looks perfect, lets duplicate it 3 times, and put one connecting each set of power pole spikes.
...
- This will create a new material called 'Seagull'. Now put the same material on to the other parts of the seagull (the wings and legs).

### Camera:
- (Skip this step if the camera is already set up nicely in the scene)
- Move the scene view camera so that you can see the bird, the power-lines, and the road, but not much else.
...
### Poop:
- Create a sphere and call it 'FallingPoop'.
- Make it a good size compared to the bird.
- Add a particle effect to the poop.
...
This will be the poop that comes out of the bird, but we don't want it to look like this when it lands on the ground, so lets make a new different poop for it to turn into when it hits something:
...
- Move and scale it to look like a lump in the squished white sphere.
	- You press 'R' or click on the icon to swap to the scale tool. Then you can click on the middle (grey) cube to scale the object evenly in all directions.
...
- You may want to look up bird poop so you can accurately re-create it!

Now we need the poop to swap between these versions when it hits something.

- Create a scripts folder.
- Create a new script called `Poop` in the scripts folder.
- Create a variable `public GameObject squishedVersion;`
- In the script, add an `OnCollisionEnter` event.
- In the event, instantiate the squished version with the current poops position and rotation.
